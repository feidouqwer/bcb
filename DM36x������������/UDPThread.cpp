    ...
    : TThread(CreateSuspended)
{
	...
	if (ret == SOCKET_ERROR)
	{
		ShowMessage("Failed to set udp broadcase.");
		return;
	}
    char ipStr[32];
	...
	{
		ShowMessage("Failed to bind server socket.");
		return;
	}
}
...
{
    Form1->lblInfo->Caption = "获取IP地址失败!!!";
    Form1->btnKernel->Enabled = false;
    Form1->btnFilesys->Enabled = false;
...
void __fastcall TUDPThread::Execute()
{
    ...
    while(!this->Terminated)
    {
        ...
        //ShowMessage("begin recv udp data");
...
int __fastcall TUDPThread::SendBroadCastPacket()
{
	...
	{
		ShowMessage("Failed to send broadcase udp packet.");
		return false;
	}
    return true;
}
int __fastcall TUDPThread::SendBroadCastNetInfo(net_struct *pNetInfo)
{
    ...
    pHead = (PSTJMsgHeader)buffer;
    pHead->uMagic = HEAD_MAGIC;
    pHead->uType = PSTJ_BROADCASE_IP_TABLE_TO_UPDATE;
    pHead->ret = 0;
    ...
    if(sendLen == SOCKET_ERROR)
    {
        Application->MessageBoxA("发送IP_TABLE UDP广播包失败", "错误", MB_OK);
        return false;;
    }
    return true;
}
